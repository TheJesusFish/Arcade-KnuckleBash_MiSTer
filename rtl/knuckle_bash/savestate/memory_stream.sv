...
typedef enum {
    IDLE, READ_MEM_REQ, READ_MEM_WAIT, READ_STREAM, WRITE_GATHER,
    WRITE_MEM_REQ, WRITE_MEM_FINAL_REQ, WRITE_MEM_FINAL_WAIT,
    WRITE_MEM_WAIT, QUERY_GATHER_FIRST, QUERY_GATHER_NEXT,
    QUERY_GATHER_WAIT, QUERY_SCATTER_WAIT, READ_HEADER,
    READ_HEADER_WAIT, WRITE_HEADER_SIZE, WRITE_HEADER_SIZE_WAIT,
    WRITE_HEADER_CHANGE, WRITE_HEADER_CHANGE_WAIT
} state_t;
...
always_comb begin
    chunk_select = 0;
    if (busy) chunk_select = 8'(chunk_index);
end
...
always_ff @(posedge clk) begin
    if (reset) begin
        ...
        case (state)
            ...
            WRITE_GATHER: begin
                if (chunk_remaining == 0) begin
                    if (word_counter != 0) state <= WRITE_MEM_REQ;
                    else state <= QUERY_GATHER_NEXT;
                end else if (chunk_data_ack & read_req) begin
                    case (chunk_width)
                        0: case (word_counter)
                        ...
                        2: case (word_counter)
                            ...
                            default: begin end
                        endcase
                        3: buffer[63:0] <= chunk_read_data[63:0];
                    endcase
