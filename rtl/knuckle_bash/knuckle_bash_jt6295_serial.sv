...
module knuckle_bash_jt6295_serial(
    input               rst,
    input               clk,
    input               cen,
    input               cen4,
...
// MAME counts both nibbles of the inclusive stop byte. Comparing only the
// byte address either drops stop-low (>=) or admits stop+1-high (>). The
// 19-bit nibble counter stops precisely after presenting stop-low.
assign over     = cnt >= {stop_out, 1'b1};
